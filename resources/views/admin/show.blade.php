...
<a href="{{ route('admin.tickets') }}">Volver</a>
    <h2>{{ $ticket->title }}</h2>
        ...
        <form action="{{ route('admin.updatePrioridad', $ticket->id) }}" method="POST">
            ...
            @method('PUT')
            <div class="mb-3">
                <label for="status" class="form-label"><strong>Prioridad:</strong></label>
                <select name="status" id="status" class="form-control">
                    <option value="media" {{ $ticket->status == 'media' ? 'selected' : '' }}>Media</option>
                    <option value="alta" {{ $ticket->status == 'alta' ? 'selected' : '' }}>Alta</option>
                    <option value="baja" {{ $ticket->status == 'baja' ? 'selected' : '' }}>Baja</option>
                </select>
            </div>
